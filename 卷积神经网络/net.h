...
public:
	...
	Net(int hide,int col,int firstnerous){
		ReadDate();
		ReadTest();
		date();
...
bool Net::ReadDate(){
	...
	while (!ifs.eof()){
		...
		arma::fmat* z = new arma::fmat(28,28);


		for (int i = 0; i < src.cols; i++){
			for (int j = 0; j<src.rows; j++){
				img.image[i*src.cols + j] = src.at<uchar>(i, j) / 255.0;
				z->at(i,j) = src.at<uchar>(i, j);
			}
			
		}
		traininput.push_back(z);
		im.push_back(img);
...
bool Net::ReadTest(){
	...
	while (!ifs.eof()){
       ...
       arma::fmat* p = new arma::fmat(28,28);
		for (int i = 0; i < src.cols; i++){

			for (int j = 0; j<src.rows; j++){
				img.image[i*src.cols + j] =src.at<uchar>(i, j) / 255.0;
				p->at(i,j) = src.at<uchar>(i, j);
			}
...
bool Net::Save(int t){
	...
	_itoa_s(0,p,10);
	string pp(p);
	string name = "save"+times+pp+".txt";
	ofstream ou(name);
	ou << h_hide << '\n';
	ou<<firstnerous<<'\n';
	for (int i = 0; i < h_hide+1; i++){
		e_hide.at(i).Save(ou,t);
	}
	ou.close();
	return true;
}
