...
void Layer::Save(ofstream &s,int i){
	    char ti[9];
		_itoa_s(i, ti, 10);
		string times(ti);
		char p[10];
		char q[10];
		_itoa_s(numofsize, p, 10);
		_itoa_s(numofbefore, q, 10);
		string qq(q);
		string pp(p);
		cout<<"szie:" <<numofbefore<<"*"<<numofsize<<'\n';
		string s1 = times+ qq + "." + pp + "aOmg.txt";
		string s2 = times+qq + "." + pp + "aWmg.txt";
		string s3 = times+qq + "." + pp + "ab.txt";
		string s4 = times+qq + "." + pp + "aBack.txt";
		s<<numofbefore<<'\n'<<numofsize<<'\n';
		s<<s1<<'\n';
		s<<s2<<'\n';
	 	s<<s3<<'\n';
